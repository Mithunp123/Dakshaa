...
const EventCoordinatorDashboard = () => {
  ...
  const generateAttendanceReport = () => {
    ...
    try {
      ...
      // Add Logos
      // KSRCT Logo (Left) - Square/Circular aspect ratio
      doc.addImage(ksrctLogo, 'WEBP', 15, 10, 25, 25);
      
      // Dakshaa Logo (Right) - Landscape aspect ratio
      doc.addImage(logo, 'WEBP', pageWidth - 45, 15, 35, 15);
      
      // Title Section
      doc.setFontSize(18);
      doc.text('K.S.Rangasamy College of Technology', pageWidth / 2, 20, { align: 'center' });
      doc.setFontSize(14);
      doc.text('Dakshaa 2025 - Coordinator Attendance Report', pageWidth / 2, 30, { align: 'center' });
      doc.setFontSize(12);
      ...
      // Table
      const tableColumn = ["S.No", "Name", "Dakshaa ID", "Team", "Attendance"];
      const tableRows = [];
      ...
      sortedParticipants.forEach((p, index) => {
        const rowData = [
          ...
          p.team_name || '-',
          (p.morningAttended || p.eveningAttended) ? 'Present' : 'Absent'
        ];
      ...
      for(let i = 1; i <= pageCount; i++) {
        ...
        doc.text('Generated by Dakshaa Admin Portal', 15, doc.internal.pageSize.height - 10);
      }

      doc.save(`Attendance_${(selectedEvent.name || 'event').replace(/\s+/g, '_')}_Report.pdf`);
    ...
    try {
      ...
      if (rpcError) {
        if (isDuplicateAttendanceError(rpcError)) {
          playBuzzSound();
          toast('⚠️ Duplicate Entry - Already marked for this session', { 
            duration: 2000, 
            icon: '⚠️', 
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            <button
               ...
               className="w-12 h-12 bg-indigo-500/10 rounded-2xl flex items-center justify-center border border-indigo-500/20 active:scale-95 transition-all text-indigo-400 hover:text-indigo-300"
            ...
            >
              ...
              <div className="space-y-3">
                {filteredParticipants.map(p => {
                  ...
                  const registeredMemberCount = p.teamMembers?.filter(m => m.morningAttended || m.eveningAttended).length || 0;
                    ...
                    {isTeamLeader && isExpanded && (
                      ...
                      <div className="ml-6 pl-4 border-l-2 border-white/10 space-y-2 pb-2">
                        ...
                        {p.teamMembers.map((member) => {
                          const memberHasCurrentSession = member.morningAttended || member.eveningAttended;
                          return (
                            <div
                              key={member.user_id}
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    {(member.morningAttended || member.eveningAttended) && <div className="mt-1"><span className="text-[10px] px-1.5 py-0.5 bg-green-500/20 text-green-500 rounded">✓ Present</span></div>}
                                  </div>
                                </div>
                                {!memberHasCurrentSession && (
                                  <button
                                    ...
                                    disabled={submitting}
                                    className="px-3 py-1.5 rounded-lg font-bold text-xs transition-all disabled:opacity-50 bg-green-500 text-white hover:bg-green-600"
                                  >
                                    Mark
                                  </button>
                                )}
