...
const MyRegistrations = () => {
  ...
  const downloadCertificate = (reg) => {
    try {
      ...
      const eventDateObj = reg.events?.event_date 
        ? new Date(reg.events.event_date)
        : new Date();
      const eventDate = `${String(eventDateObj.getDate()).padStart(2, '0')}.${String(eventDateObj.getMonth() + 1).padStart(2, '0')}.${eventDateObj.getFullYear()}`;
